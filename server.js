...
// ✅ 你要求：header/footer 不變，只在 body 插入「感情問題清單」並且隨機分配到四頁
// ✅ 不動結構：postback data 仍是 action=choose_service&service=xxx
// ------------------------------------------------------------
async function sendServiceSelectFlex(userId) {
  /***************************************
   * [0] 定義服務清單（保留你原本結構）
   * - header/footer 你說不變：所以這段維持你原本的寫法
   ***************************************/
  const services = [
    {
      id: "name",
    ...
    },
    {
      ...
      label: "紫微斗數",
      intro:
        "紫微是你人生可能會走的路徑，這條路上有崎嶇不平之處，也有羊腸小徑之途\n用更細的視角看關係互動與人生節點。",
      highlights: [
        "想知道你的角色定位與人際互動模式",
        "想知道你的愛情觀(感情好嗎)",
        "想知道你的金錢觀(有偏財嗎)",
   ...
   * [2] 隨機打散（Fisher–Yates）
   * ✅ 為什麼要這樣做：
   * - 每次使用者點「預約」看到的問題排列不一樣，比較像「有在對話」
   * - 你要求「隨機放入四張」→ 先 shuffle 最乾淨
   ***************************************/
   ...
   * [3] 把問題切成四份（對應四頁）
   * ✅ 為什麼這樣做：
   * - 不改 header/footer 的前提下，把「引導提問」塞進 body
   * - 每頁看到不同問題 → 使用者更容易覺得「這頁就是我需要的」
   ***************************************/
  const shuffled = shuffle(loveQuestions);
  const perPage = Math.max(1, Math.ceil(shuffled.length / services.length)); // 16/4=4
  const questionsByPage = services.map((_, idx) =>
    shuffled.slice(idx * perPage, (idx + 1) * perPage)
  );

  /***************************************
   * [4] 產生每一頁 bubble（一服務一頁）
   * ✅ 你要求：header/footer 不變
   * ✅ 我只在 body 多插一段「常見感情問題」（而且樣式跟你指定的一樣）
   ***************************************/
  const bubbles = services.map((s, idx) => {
    // ✅ 亮點 bullets：你原本的樣式（不動）
    ...
    /***************************************
     * ✅ 把該頁要顯示的「感情問題」做成文字元件
     * ✅ 你要求：字體外觀全部用「適合什麼樣的人？」那一行的樣式
     * - size: "sm"
     * - weight: "bold"
     * - color: "#111111"
     ***************************************/
    const questionTexts = (questionsByPage[idx] || []).map((q) => ({
      type: "text",
      text: `• ${q}`,
      size: "sm",
      weight: "bold",
    ...
    return {
      ...
      /***************************************
       * [4-2] Body：你原本的「intro + highlights」
       * ✅ 只新增一段「常見感情問題」並放在 highlights 後面
       ***************************************/
      body: {
        type: "box",
        layout: "vertical",
