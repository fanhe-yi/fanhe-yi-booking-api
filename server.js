...
// 🔹 第一步：服務選擇 Flex（Carousel：六爻 / 姓名學 / 紫微 / 八字）
// ------------------------------------------------------------
// ✅ 目的：把原本「四顆按鈕」改成「四頁 Carousel」
// ✅ 不動後端結構：postback data 仍是 action=choose_service&service=xxx
// ✅ UI 結構：header/footer 不變、body 只放你分類好的 questions
// ------------------------------------------------------------
async function sendServiceSelectFlex(userId) {
  /***************************************
   * [0] 定義服務清單（你提供的版本）
   * - 注意：questions 裡每一項都必須是字串（要有引號）
   ***************************************/
  const services = [
    {
      id: "liuyao",
   ...
   * - header：標題 + badges + separator +「適合什麼樣的人？」
   * - body：只放 questions（字體放大一級）
   * - footer：CTA 功能不變（走 choose_service）
   ***************************************/
  const bubbles = services.map((s) => {
    ...
    return {
      ...
      /***************************************
       * [1-2] Body：只放 questions（不放 intro/highlights）
       ***************************************/
      body: {
        type: "box",
        layout: "vertical",
        ...
        contents: [
          ...
          {
            type: "text",
            text: "🔒 想看更貼近你的狀況：分享後回來按下方第二顆即可解鎖",
            size: "xs",
            color: "#666666",
            wrap: true,
            margin: "md",
