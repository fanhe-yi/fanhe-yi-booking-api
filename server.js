...
// ✅ 優點：更像在賣服務、可讀性高、用戶滑一滑就想點預約
// ✅ 不動結構：postback 的 data 仍是 action=choose_service&service=xxx
// ------------------------------------------------------------
async function sendServiceSelectFlex(userId) {
  /***************************************
   * [0] 定義服務清單（保留原本 id 結構）
   * - id：你後端用來辨識服務用（不要改，除非你後端也改）
   * - label：按鈕文字與顯示用
   * - intro：每頁簡短吸睛文案（像產品卡）
   * - highlights：3 個 bullet，讓人一眼抓到「你到底能幫我什麼」
   ***************************************/
  const services = [
    {
      id: "bazi",
  ...
  /***************************************
   * [2] Carousel：一次送出 4 頁
   * - LINE Flex 規格：contents 必須是 bubbles array
   ***************************************/
  const flexPayload = {
    type: "carousel",
    contents: bubbles,
