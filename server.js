...
const liuyao = makeLiuyao({
  ...
  LIU_YAO_TOPIC_LABEL,
  toTW, // 如果你有用到才塞
  sendGenderSelectFlex, // 你 routePostback 會用到
  sendLiuYaoTimeModeFlex, // 六爻選 now/custom
});
...
async function routeByConversationState(userId, text, state, event) {
  ...
  // 新增：六爻占卜
  if (mode === "liuyao") {
    return await liuyao.handleFlow(userId, text, state, event);
  }
...
async function routePostback(userId, data, state) {
  ...
  // ============================
  // ✅ 六爻：所有 postback action 統一交給 liuyao.js
  // ============================
  if (action && action.startsWith("liuyao_")) {
    return await liuyao.handlePostback(userId, data, state);
  }
