...
// 🔹 第一步：服務選擇 Flex（Carousel：八字 / 紫微 / 姓名 / 六爻）
// ------------------------------------------------------------
// ✅ 目的：把原本「四顆按鈕」改成「四頁產品型介紹」
// ✅ 不動結構：postback 的 data 仍是 action=choose_service&service=xxx
// ✅ 你指定：header / footer 不變（我只動 body）
// ✅ 你指定：把「適合什麼樣的人？」往上拉到分隔線下面；intro/highlights 刪掉
// ✅ 你指定：四頁各放一組「常見問題」文字（我直接分配，日後你自己換就好）
// ------------------------------------------------------------
async function sendServiceSelectFlex(userId) {
  /***************************************
   * [0] 定義服務清單（保留原本 id 結構）
   * - id：你後端辨識服務用（不要改，除非你後端也改）
   * - label：顯示用
   * - badges：header 用（你之前已改成文字 badge，這裡沿用）
   * - ctaLabel / ctaDisplay：footer 用（你說 header/footer 不變，所以我沿用你先前「依服務類型配一句」的做法）
   * - questions：你給的「常見問題」文字，我直接分四組塞進四頁（不做隨機）
   ***************************************/
  ...
  const services = [
    ...
    },
    {
      id: "liuyao",
      label: "六爻占卜",
      badges: ["像去廟裡問事一樣", "我有件事想用占卜看一下"],
      ctaLabel: "我想問這個",
      ctaDisplay: "我想先聊聊：六爻占卜",
      questions: [
        "• 現在的曖昧關係會往下一步發展嗎？",
        "• 我該主動表達還是等待更好的時機？",
        "• 這段關係該不該繼續走下去？",
  ...
  /***************************************
   * [1] 產生每一頁 bubble（一服務一頁）
   * - ✅ header：不動（沿用你現在的版本：標題 + badges）
   * - ✅ footer：不動（沿用你現在的版本：同功能 postback）
   * - ✅ body：重做成「separator 下方立刻出現『適合什麼樣的人？』＋問題清單」
   ***************************************/
  const bubbles = services.map((s) => {
    // ✅ 你給的問題清單：直接轉成 Flex text
    ...
    return {
      ...
      /***************************************
       * [1-2] Body：只保留「分隔線」＋「適合什麼樣的人？」＋「問題清單」
       * - intro / highlights 你指定刪掉
       * - 「適合什麼樣的人？」你指定拉到 separator 下面
       ***************************************/
      body: {
        type: "box",
        layout: "vertical",
      ...
      /***************************************
       * [1-3] Footer：保留不變（功能一樣）
       * - data: action=choose_service&service=xxx
       * - displayText: 你擔心 state 跳掉，這裡仍是 postback，不會跳
       ***************************************/
      footer: {
        type: "box",
        layout: "vertical",
        ...
        contents: [
          {
            ...
            action: {
              type: "postback",
              label: s.ctaLabel, // ✅ 依服務類型配一句 CTA
              data: `action=choose_service&service=${s.id}`,
              displayText: s.ctaDisplay, // ✅ 依服務類型配一句（功能不變）
            },
          },
        ],
      },
